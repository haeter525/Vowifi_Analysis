...
columnToDrop = [
    ...
    if col not in ['Direct', 'Length', 'Event', 'preEvent']
]
dataset = dataset.drop(columns=columnToDrop)

dataset = (
    dataset.drop(dataset[(dataset.Length < 190)].index)
    .reset_index()
    .drop(columns='index')
)
print('完成！')
...
# 驗證資料
assert (
    all(dataset['Direct'].str.match(pat='Direction\.((UPWARD)|(DOWNWARD))')),
    'Direct 欄位有不正確的值',
)
...
print(
    f'目前時間:{currentTime.tm_mon:>2}月{currentTime.tm_mday:>2}日{currentTime.tm_hour:>2}點{currentTime.tm_min:>2}分{currentTime.tm_sec:0>2}秒'
)
...
testset = (
    testset.drop(testset[testset.Length < 190].index)
    .reset_index()
    .drop(columns='index')
)
